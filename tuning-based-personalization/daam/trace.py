...
class PipelineHooker(ObjectHooker[StableDiffusionPipeline]):
    ...
    def _hooked_run_safety_checker(hk_self, self: StableDiffusionPipeline, image, *args, **kwargs):
        ...
        #print(image)
        #image_numpy = image.cpu().squeeze(0).detach().numpy()
        #image_numpy = image.cpu().permute(0,3,1,2).detach().numpy()
        image_numpy = image.cpu().detach().numpy()
        image_numpy = ((image_numpy.transpose(0,2,3,1)+1)/2)
        pil_image = self.numpy_to_pil(image_numpy)
