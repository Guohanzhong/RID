...
def main(_):
    ...
    #    unet, r=config.train.lora_rank, loras=None
    #)
    ...
    unet_lora_params = AttnProcsLayers(unet.attn_processors)

    if config.lora_path is not None:
        if config.use_lora:
            token_savepath = os.path.join(config.lora_path,"<new1>.bin")
            unet.load_attn_procs(config.lora_path)
            pipeline.load_textual_inversion(token_savepath)
            logger.info('Sucessfully loading the pre-trained lora')
        else:
        ...
        generator = torch.Generator(device=accelerator.device).manual_seed(40234212) 
        prompt = "photo of a <new1> person"
        images = pipeline(prompt=prompt,generator=generator).images[0]
    ...
    if config.use_lora:
        if config.db_info.modifier_token is not None:
            params_to_optimize = (
                ...
                if config.train.train_text_encoder
                else itertools.chain(unet_lora_params.parameters(),text_encoder_one.get_input_embeddings().parameters())
            )
        else:
            params_to_optimize = (
                [
    ...
    loss_dict = {timestep: [] for timestep in range(1, 1000)}
    grad_dict = {timestep: [] for timestep in range(1, 1000)}
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        #)
        for timestep in trange(1, 1000):
        #timesteps = torch.cat([timesteps,timesteps],dim=0)
        #timesteps = timesteps.long()
            timesteps = torch.tensor([timestep], device=latents.device).long()
            ...
            loss_dict[timestep].append(avg_loss.item())
            ...
            for param in unet.parameters():
                if param.grad is not None:
                    grads.append((param.grad.clone()).pow(2).detach().cpu().numpy().mean())
                else:
                    grads.append(0.0)
            import numpy as np
            grads_mean = np.mean(grads)
            print(grads_mean)
            grad_dict[timestep].append(grads_mean)

            optimizer.zero_grad()

        # Calculate average loss and gradients for each timestep
        avg_loss_dict = {timestep: sum(losses) / len(losses) for timestep, losses in loss_dict.items()}
        #avg_grad_dict = {timestep: [sum(grad) / len(grad) for grad in zip(*grad_dict[timestep])] for timestep in grad_dict.keys()}
        avg_grad_dict = {timestep: sum(losses) / len(losses) for timestep, losses in grad_dict.items()}

        # Save the average loss and gradient dictionaries as JSON files
        save_exp_name = config.outtest_exp_name
        os.makedirs(save_exp_name, exist_ok=True)
        loss_dict_path = os.path.join(save_exp_name,"avg_loss_dict.json")
        grad_dict_path = os.path.join(save_exp_name,"avg_grad_dict.json")
        #grad_dict_path = '/root/allan/GA/tuning-based-personalization/results_json/avg_grad_dict_c.json'
        #loss_dict_path = '/root/allan/GA/tuning-based-personalization/results_json/avg_loss_dict_c.json'
        #grad_dict_path = '/root/allan/GA/tuning-based-personalization/results_json/avg_grad_dict_c.json'

        with open(loss_dict_path, 'w') as loss_file:
            json.dump(avg_loss_dict, loss_file, indent=4)

        with open(grad_dict_path, 'w') as grad_file:
            json.dump(avg_grad_dict, grad_file, indent=4)
