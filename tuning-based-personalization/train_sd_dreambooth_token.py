...
def main(_):
    ...
    if config.db_info.modifier_token is not None:
        params_to_optimize = (
            [
                {"params": itertools.chain(unet.parameters()), "lr": config.train.learning_rate},
                {"params": itertools.chain(*text_encoder_one.get_input_embeddings().parameters()),"lr": config.train.learning_rate,},
                {"params": itertools.chain(text_encoder.parameters()),"lr": config.train.learning_rate,},
            ]
            if config.train.train_text_encoder
            else itertools.chain(unet.parameters(),text_encoder_one.get_input_embeddings().parameters())
        )
    else:
        params_to_optimize = (
            [
    ...
    for epoch in range(config.train.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            target = noise
            # Dreambooth will use some instances to train a specific personalization model;
            ...
            if accelerator.sync_gradients:
                ...
                if global_step != 0 and global_step % config.train.save_steps == 0 and accelerator.is_main_process:
                    if accelerator.is_main_process:
                        ...
                        filename_safe = f"{save_path}/unet_weight.safetensors"
            ...
            if accelerator.is_main_process:
                if config.db_info.validation_prompt is not None and global_step % config.db_info.validation_epochs == 0:
                    ...
                    generator = torch.Generator(device=accelerator.device).manual_seed(config.seed+global_step//10) if config.seed else None
                    images = [
                    ...
                    if sample2:
                        if config.db_info.modifier_token is not None:
                            ...
                            images = [
                                pipeline_sample("photo of a sks person",generator=generator).images[0]
                                for _ in range(config.db_info.num_validation_images)
                            ]
                        os.makedirs(config.img_output_dir, exist_ok=True)
