...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="image/train_image_anna",
        required=False,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="text-inversion-model_random",
...
def pgd_attack(
    ...
    for step in range(num_steps):
        ...
        grad_random = (torch.randn(size=(perturbed_images.shape)).to(perturbed_images.device) + 1 ) * 0.3 - 0.5* (torch.randn(size=(perturbed_images.shape)).to(perturbed_images.device)).pow(2)
        grad = perturbed_images.grad.detach().clone()
        grad[:9] = grad[1:].clone()
        ...
        logging.info(f"PGD loss - step {step} and the timestep is {timesteps.float().mean()}")
