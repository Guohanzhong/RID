...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="image/n000050_data/train",
        required=False,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="./EXP/Uni_attack_5e-6_16t255_n50_t0t800_sn17_data-n000050",
...
def pgd_attack(
    ...
    for step in range(num_steps):
        for k in range(perturbed_images.shape[0]):
            #input_ids = input_ids.repeat(14,1)

            perturbed_images_temp = perturbed_images[k,...].unsqueeze(0).repeat(space_num,1,1,1)
            ...
            bsz = latents.shape[0]
            # Sample a random timestep for each image
            timesteps = torch.randint(0, int(noise_scheduler.config.num_train_timesteps*0.8), (bsz,), device=latents.device)
            timesteps = timesteps.long()
            # Add noise to the latents according to the noise magnitude at each timestep
            # (this is the forward diffusion process)
            noisy_latents = noise_scheduler.add_noise(latents, noise, timesteps)
        ...
        logging.info(f"PGD loss - step {step+1}, loss: {loss.detach().item()} and the timestep is {timesteps.float().mean()}, the gap is {(torch.abs(perturbed_images[0,...]-original_images[0,...])-torch.abs(perturbed_images[1,...]-original_images[1,...])).max()}")
