...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--pretrained_model_name_or_path",
        type=str,
        default="stable-diffusion-2-1-base,stable-diffusion-v1-5",
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="attack_raw/eps-12_255_advdm_rep2",
...
def parse_image_name(image_name):
    a, b = image_name.split('_')[0], image_name.split('_')[1]
...
class IMAGE(Dataset):
    def __init__(self, path,image_size=512):
        if "json" in path:
            ...
            sorted_images = sorted(self.images, key=lambda img: parse_image_name(osp.basename(img)))
...
def pgd_attack(
    ...
    for step in range(num_steps):
        ...
        if accelerator.is_main_process:
            ...
            logging.info(f"eta max {eta.max()}, eta min {eta.min()}")
...
def main(args):
    ...
    for epoch in range(maxepoch):
        ...
        for i in range(args.max_train_steps):
            ...
            logging.info(f'max eps {(perturbed_data-original_data).max()}')    
            ...
            if (i + 1) % args.checkpointing_iterations == 0:
                ...
                original_data_gather = accelerator.gather(original_data.contiguous())
                save_folder_clean = f"{args.output_dir}/{epoch}_clean/{i+1}"
                os.makedirs(save_folder_clean, exist_ok=True)

                if accelerator.is_main_process:
                    noised_imgs = perturbed_data_gather.detach()
                    clean_imgs = original_data_gather.detach()
                    ...
                    for n_img,img_pixel in enumerate(noised_imgs):
                        ...
                        logging.info(f"image range is {img_pixel.min()},{img_pixel.max()}")
                        logging.info(f'max eps finally {((img_pixel * 127.5 + 128)-(clean_imgs[n_img,...] * 127.5 + 128)).max()}')    

                    for n_img,img_pixel in enumerate(clean_imgs):
                        save_path = os.path.join(save_folder_clean, f"clean_{str(n_img)}.png")
                        Image.fromarray(
                            (img_pixel * 127.5 + 128).clamp(0, 255).to(torch.uint8).permute(1, 2, 0).cpu().numpy()
                        ).save(save_path)
