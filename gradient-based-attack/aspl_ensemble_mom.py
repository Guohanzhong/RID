...
def load_model(args, model_path,accelerator):
    ...
    if args.enable_xformers_memory_efficient_attention:
        ...
        logging.info("Make sure to install the following packages before continue")
        logging.info("pip install triton==2.0.0.dev20221031")
        logging.info("pip install pip install xformers==0.0.17.dev461")
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--pretrained_model_name_or_path",
        type=str,
        default="stable-diffusion-v1-4,stable-diffusion-v1-5",
    ...
    parser.add_argument(
        ...
        type=str,
        default="vgg/selected_images.json",
        #default="/2d-cfs-nj/alllanguo/code/TEST/Pyguard/image/n000050_data/train",
    ...
    parser.add_argument(
        "--class_data_dir",
        type=str,
        default="./image/class_people_sd_new",
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="large-scale-exp/eps-12_255-mom_anti-a9f0",
        #default="/2d-cfs-nj/alllanguo/code/TEST/Attack/image/eps-8_255-mom_anti",
...
class IMAGE(Dataset):
    def __init__(self, path,image_size=512):
        if "json" in path:
            with open(path, 'r') as f:
                selected_images_dict = json.load(f)
            all_image_paths = []
            for subdir_name in selected_images_dict:
                all_image_paths.extend(selected_images_dict[subdir_name])
                self.images = all_image_paths
        else:
            print(path)
            self.path = path
    ...
    def __getitem__(self, index):
        ...
        images = self.image_transforms(Image.open(path).convert("RGB"))
        return images
...
def train_one_epoch(
    ...
    step_data = train_dataset[0 % len(train_dataset)]
    device = step_data["instance_images"].device
    ...
    for step in range(num_steps):
        ...
        #logging.info(step_data["instance_images"].device)
        #logging.info(step_data["instance_prompt_ids"].device)
        #logging.info(step_data["class_images"].device)
        pixel_values = torch.stack([step_data["instance_images"], step_data["class_images"].to(device)]).to(
        ...
        timesteps = torch.randint(0, int(noise_scheduler.config.num_train_timesteps*0.9), (bsz,), device=latents.device)
        ...
        del model_pred,encoder_hidden_states,noisy_latents
...
def pgd_attack(
    ...
    gradient_accumulated: torch.Tensor,
    num_steps: int,
    accelerator
):
    ...
    for step in range(num_steps):
        ...
        x_grad = perturbed_images.grad
        gradient_accumulated = 0.99*gradient_accumulated.data + x_grad / torch.mean(torch.abs(x_grad), dim=(1,2,3), keepdim=True)
        x_grad = gradient_accumulated.clone().detach()

        alpha = args.pgd_alpha
        eps = args.pgd_eps

        adv_images = perturbed_images + alpha * x_grad.sign()
        eta = torch.clamp(adv_images - original_images, min=-eps, max=+eps)
        perturbed_images = torch.clamp(original_images + eta, min=-1, max=+1).detach_()
        if accelerator.is_main_process:
            logging.info(f"PGD loss - step {step}, loss: {loss.detach().item()}")
            
    return perturbed_images,gradient_accumulated
...
def main(args):
    ...
    dataset = IMAGE(args.instance_data_dir_for_train,image_size=args.resolution)
    train_dataset_loader = DataLoader(dataset, batch_size=args.train_batch_size, shuffle=False, drop_last=True,
                                      num_workers=8, pin_memory=True, persistent_workers=True)
    train_dataset_loader = accelerator.prepare(train_dataset_loader)
    maxepoch = max(len(dataset) // (accelerator.num_processes * args.train_batch_size),1)
    if accelerator.is_local_main_process:
        logging.info(f'the number of epoch is {maxepoch} and the number of samples is {len(dataset)}')

    def get_data_generator():
        while True:
            for x in iter(train_dataset_loader):
                yield x
    ...
    for epoch in range(maxepoch):
        clean_data = next(data_generator)
        perturbed_data = clean_data
        original_data = perturbed_data.clone()
        original_data.requires_grad_(False)

        g = torch.zeros(original_data.size(0), 1, 1, 1).to(original_data.device)
        ...
        for i in range(args.max_train_steps):
            ...
            for j, model_path in enumerate(model_paths):
                ...
                #text_encoder, unet, tokenizer, noise_scheduler, vae = accelerator.prepare(text_encoder, unet, tokenizer, noise_scheduler, vae)
                f = [unet, text_encoder]
            ...
            if accelerator.is_main_process:
                logging.info(f'the itrations is {i}')                
            # update
            perturbed_data = en_data.cuda()

            if (i + 1) % args.checkpointing_iterations == 0:
                #logging.info(perturbed_data.device,perturbed_data.is_contiguous())
                perturbed_data_gather = accelerator.gather(perturbed_data.contiguous())
                save_folder = f"{args.output_dir}/{epoch}/{i+1}"
                os.makedirs(save_folder, exist_ok=True)

                if accelerator.is_main_process:
                    noised_imgs = perturbed_data_gather.detach()
                    #img_names = [
                    #    str(instance_path).split("/")[-1]
                    #    for instance_path in list(Path(args.instance_data_dir_for_train).iterdir())
                    #]
                    for n_img,img_pixel in enumerate(noised_imgs):
                        save_path = os.path.join(save_folder, f"noise_{str(n_img)}.png")
